...
public class WheatFarmBot {
    ...
    int cachedMatureWheat;

    BlockPos targetWheatCrop;
    BlockPos targetWheatFarmland;
    ...
    public void reset() {
        ...
        targetWheatCrop = null;
        targetWheatFarmland = null;
    ...
    public void tick(Context ctx) {
        ...
        if (targetWheatCrop != null && tryHandleHarvestTarget(ctx, targetWheatCrop)) {
            return;
        }

        if (targetWheatFarmland != null && hasSeedsAnywhere() && tryHandlePlantTarget(ctx, targetWheatFarmland)) {
            return;
        }
    ...
    public int getCachedMatureWheat() {
        return cachedMatureWheat;
    }

    public boolean hasAnyCultureToDeposit() {
        return countItemInInventory(Items.WHEAT) > 0 || countSeedsOverReserve() > 0;
    }

    public int countSeedsOverReserve() {
        int reserve = getDynamicSeedReserve();
        return Math.max(0, countItemInInventory(Items.WHEAT_SEEDS) - reserve);
    }
    ...
    public boolean shouldGoToStorageNow() {
        ...
        int wheat = countItemInInventory(Items.WHEAT);
        int seeds = countItemInInventory(Items.WHEAT_SEEDS);
        ...
        boolean wheatWorthDelivering = wheat >= 64;
        boolean tooManySeeds = seedsOverReserve >= EXTRA_SEED_DELIVER_THRESHOLD;

        if (cachedEmptyFarmland > 0) {
            if (inventoryTight && wheat > 0) {
                return true;
            }
            return wheatWorthDelivering && inventoryTight;
        }

        if (wheatWorthDelivering) {
            return true;
        }

        if (inventoryTight && (wheat > 0 || seedsOverReserve > 0)) {
            return true;
        }

        return tooManySeeds && cachedMatureWheat <= 0;
    }
    ...
    public boolean dropNextDepositableCultureFromWholeInventory() {
        ...
        int wheatSlot = findInventorySlotPreferWhole(Items.WHEAT);
        if (wheatSlot >= 0) {
            return throwInventorySlot(wheatSlot, true);
        }

        int seedReserve = getDynamicSeedReserve();
        int seedsTotal = countItemInInventory(Items.WHEAT_SEEDS);
        int seedsOver = Math.max(0, seedsTotal - seedReserve);
        if (seedsOver <= 0) {
            return false;
        }

        int wholeDropSeedSlot = findInventorySlotPreferWholeAtMost(Items.WHEAT_SEEDS, seedsOver);
        if (wholeDropSeedSlot >= 0) {
            return throwInventorySlot(wholeDropSeedSlot, true);
        }

        int seedSlot = findInventorySlotPreferWhole(Items.WHEAT_SEEDS);
        if (seedSlot < 0) {
            return false;
        }
    ...
    private int getDynamicSeedReserve() {
        ...
        if (cachedMatureWheat > 0 && cachedEmptyFarmland <= 0) {
    ...
    private void refreshTargets(Context ctx) {
        ...
        if (targetWheatCrop != null && !isValidWheatCropTarget(ctx, targetWheatCrop)) {
            targetWheatCrop = null;
        }
        if (targetWheatFarmland != null && !isValidWheatFarmlandTarget(ctx, targetWheatFarmland)) {
            targetWheatFarmland = null;
        }
        ...
        boolean needCrop = targetWheatCrop == null;
        boolean needPlant = hasSeedsAnywhere() && targetWheatFarmland == null;
    ...
    private void scanLocalTargets(Context ctx) {
        ...
        double bestCrop = targetWheatCrop != null ? playerPos.squaredDistanceTo(getInteractPoint(targetWheatCrop)) : Double.MAX_VALUE;
        double bestFarmland = targetWheatFarmland != null ? playerPos.squaredDistanceTo(Vec3d.ofCenter(targetWheatFarmland.up())) : Double.MAX_VALUE;
        ...
        for (int y = fromY; y <= toY; y++) {
            for (int x = fromX; x <= toX; x++) {
                for (int z = fromZ; z <= toZ; z++) {
                    ...
                    if (d < bestFarmland) {
                        bestFarmland = d;
                        targetWheatFarmland = farmland.toImmutable();
                    }
                }
            }
        }
    ...
    private void scanFullTargets(Context ctx) {
        ...
        targetWheatCrop = bestCropPos;

        if (hasSeedsAnywhere()) {
            targetWheatFarmland = bestFarmlandPos;
        } else {
            targetWheatFarmland = null;
        }
    }
    ...
    private boolean tryHandleHarvestTarget(Context ctx, BlockPos cropPos) {
        if (!isValidWheatCropTarget(ctx, cropPos)) {
            targetWheatCrop = null;
            return false;
        }
    ...
    private boolean tryHandlePlantTarget(Context ctx, BlockPos farmlandPos) {
        if (!isValidWheatFarmlandTarget(ctx, farmlandPos)) {
            targetWheatFarmland = null;
            return false;
        }
    ...
    private boolean isValidWheatCropTarget(Context ctx, BlockPos pos) {
        ...
        return isMatureWheat(mc.world.getBlockState(pos));
    }

    private boolean isValidWheatFarmlandTarget(Context ctx, BlockPos pos) {
        if (pos == null || mc.world == null) {
            return false;
        }
    ...
    private boolean tryHarvestWheat(BlockPos cropPos) {
        if (mc.player == null || mc.interactionManager == null || mc.world == null) {
            return false;
        }
        if (!isMatureWheat(mc.world.getBlockState(cropPos))) {
            return false;
        }
    ...
    private boolean tryPlantWheat(BlockPos farmlandPos) {
        if (mc.player == null || mc.interactionManager == null || mc.world == null) {
            return false;
        }
        ...
        int seedSlot = findHotbarSlot(Items.WHEAT_SEEDS);
        if (seedSlot < 0) {
            if (!pullSeedsToHotbar()) {
                return false;
            }
            seedSlot = findHotbarSlot(Items.WHEAT_SEEDS);
            if (seedSlot < 0) {
                return false;
            }
        }
    ...
    private boolean hasSeedsInHotbar() {
        return findHotbarSlot(Items.WHEAT_SEEDS) >= 0;
    }

    private boolean hasSeedsAnywhere() {
        return countItemInInventory(Items.WHEAT_SEEDS) > 0;
    }
    ...
    private boolean pullSeedsToHotbar() {
        ...
        if (findHotbarSlot(Items.WHEAT_SEEDS) >= 0) {
            return true;
        }
        ...
        int seedInvSlot = findInventorySlot(Items.WHEAT_SEEDS, 9, 36);
        if (seedInvSlot < 0) {
            return false;
        }
    ...
    private int findBestHotbarSlotForSeeds() {
        ...
        for (int i = 0; i < 9; i++) {
            ItemStack s = mc.player.getInventory().getStack(i);
            if (s.isOf(Items.WHEAT_SEEDS)) {
                return i;
            }
            if (s.isOf(Items.WHEAT)) {
                continue;
            }
            if (isFoodStack(s)) {
                continue;
    ...
    private boolean isMatureWheat(BlockState state) {
        if (state == null || !state.isOf(Blocks.WHEAT)) {
            return false;
        }
