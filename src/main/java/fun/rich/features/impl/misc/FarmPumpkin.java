...
public class FarmPumpkin {
    ...
    int cachedHarvestablePumpkins;

    BlockPos targetPumpkin;

    BlockPos pickupFocusPumpkinPos;
    ...
    public void reset() {
        ...
        cachedHarvestablePumpkins = 0;

        targetPumpkin = null;

        pickupFocusPumpkinPos = null;
    ...
    public void tick(Context ctx) {
        ...
        if (targetPumpkin != null && tryHandleHarvestTarget(ctx, targetPumpkin)) {
            return;
        }
    ...
    public int getCachedHarvestablePumpkins() {
        return cachedHarvestablePumpkins;
    }

    public boolean hasAnyCultureToDeposit() {
        return countItemInInventory(Items.PUMPKIN) > 0;
    }

    public boolean shouldGoToStorageNow() {
        int pumpkins = countItemInInventory(Items.PUMPKIN);
        if (pumpkins <= 0) {
            return false;
        }

        boolean inventoryTight = getFreeInventorySlots(36) <= 1;
        boolean enoughPumpkins = pumpkins >= 32;
        boolean lotsOfPumpkins = pumpkins >= 64;
        boolean noMoreTargetsNow = cachedHarvestablePumpkins <= 0;
        ...
        if (enoughPumpkins && noMoreTargetsNow) {
            return true;
        }

        return enoughPumpkins;
    }
    ...
    public boolean dropNextDepositableCultureFromWholeInventory() {
        ...
        int pumpkinSlot = findInventorySlotPreferWhole(Items.PUMPKIN);
        if (pumpkinSlot < 0) {
            return false;
        }

        return throwInventorySlot(pumpkinSlot, true);
    }

    private void refreshTargets(Context ctx) {
        long now = System.currentTimeMillis();

        if (targetPumpkin != null && !isValidPumpkinTarget(ctx, targetPumpkin)) {
            if (targetPumpkin.equals(breakingPumpkinPos)) {
                resetBreakingState(true);
            }
            targetPumpkin = null;
        }
        ...
        if (targetPumpkin == null && now - lastFullRescanAt >= FULL_RESCAN_INTERVAL_MS) {
    ...
    private void scanLocalTargets(Context ctx) {
        ...
        double best = targetPumpkin != null ? playerPos.squaredDistanceTo(getInteractPoint(targetPumpkin)) : Double.MAX_VALUE;
        ...
        for (int y = fromY; y <= toY; y++) {
            for (int x = fromX; x <= toX; x++) {
                for (int z = fromZ; z <= toZ; z++) {
                    ...
                    if (!isHarvestablePumpkin(state)) {
                        continue;
                    }

                    if (!isValidPumpkinTarget(ctx, pos)) {
                        continue;
                    }
    ...
    private void scanFullTargets(Context ctx) {
        ...
        targetPumpkin = bestPos;
    }

    private boolean tryHandleHarvestTarget(Context ctx, BlockPos pumpkinPos) {
        if (!isValidPumpkinTarget(ctx, pumpkinPos)) {
            if (pumpkinPos.equals(breakingPumpkinPos)) {
                resetBreakingState(true);
            }
            targetPumpkin = null;
            return false;
        ...
        if (canReachBlock(pumpkinPos)) {
            ctx.stopWorkPathing();
            resetTrackedGoal();

            boolean handled = tryHarvestPumpkin(pumpkinPos);
            if (!handled) {
                return false;
            }
            ...
            if (!isValidPumpkinTarget(ctx, pumpkinPos)) {
                targetPumpkin = null;
                pickupFocusPumpkinPos = pumpkinPos.toImmutable();
        ...
        if (breakingPumpkinPos != null) {
            resetBreakingState(true);
        }

        BlockPos approach = findApproachPosForPumpkin(ctx, pumpkinPos);
        if (approach == null) {
            pathToGoalTracked(ctx, findFallbackGoalNearPumpkin(ctx, pumpkinPos));
            return true;
        }
    ...
    private boolean tryHandlePickupPhase(Context ctx) {
        if (pickupFocusPumpkinPos == null || mc.player == null || mc.world == null) {
            return false;
        }
        ...
        ItemEntity drop = findNearestRelevantDropAround(pickupFocusPumpkinPos, ITEM_SEARCH_RADIUS);
        if (drop == null) {
            return false;
        }
    ...
    private boolean isValidPumpkinTarget(Context ctx, BlockPos pos) {
        ...
        return isHarvestablePumpkin(mc.world.getBlockState(pos));
    }

    private boolean tryHarvestPumpkin(BlockPos pumpkinPos) {
        if (mc.player == null || mc.interactionManager == null || mc.world == null || pumpkinPos == null) {
            return false;
        }

        if (!isHarvestablePumpkin(mc.world.getBlockState(pumpkinPos))) {
            if (pumpkinPos.equals(breakingPumpkinPos)) {
                resetBreakingState(false);
            }
            return false;
        ...
        selectBestAxeForPumpkinIfAvailable();

        Direction dir = getBestBreakDirection(pumpkinPos);
        long now = System.currentTimeMillis();

        try {
            if (breakingPumpkinPos == null || !breakingPumpkinPos.equals(pumpkinPos)) {
                if (now < nextActionAt) {
                    return false;
                }
                ...
                lookAt(getInteractPoint(pumpkinPos));

                mc.interactionManager.attackBlock(pumpkinPos, dir);
                mc.player.swingHand(Hand.MAIN_HAND);

                breakingPumpkinPos = pumpkinPos.toImmutable();
            ...
            boolean progressed;
            try {
                progressed = mc.interactionManager.updateBlockBreakingProgress(pumpkinPos, dir);
    ...
    private void selectBestAxeForPumpkinIfAvailable() {
    ...
    private BlockPos findApproachPosForPumpkin(Context ctx, BlockPos pumpkinPos) {
        return findApproachPosForInteract(ctx, pumpkinPos, pumpkinPos);
    }

    private BlockPos findFallbackGoalNearPumpkin(Context ctx, BlockPos pumpkinPos) {
        if (ctx == null || pumpkinPos == null || mc.world == null) {
            return pumpkinPos;
        }
        ...
        for (Direction d : new Direction[]{Direction.NORTH, Direction.SOUTH, Direction.WEST, Direction.EAST}) {
            BlockPos p = pumpkinPos.offset(d);
            if (!ctx.isInsideAreaMargin(p, 2)) {
                continue;
            }
            if (!isStandablePos(p)) {
        ...
        return best != null ? best : pumpkinPos;
    }
    ...
    private Vec3d getInteractPoint(BlockPos pos) {
        ...
        if (state.isOf(Blocks.PUMPKIN)) {
            return new Vec3d(pos.getX() + 0.5D, pos.getY() + 0.55D, pos.getZ() + 0.5D);
        }
    ...
    private boolean isHarvestablePumpkin(BlockState state) {
        return state != null && state.isOf(Blocks.PUMPKIN);
    }
}