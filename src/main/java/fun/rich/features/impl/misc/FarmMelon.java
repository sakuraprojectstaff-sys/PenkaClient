...
public class FarmMelon {
    ...
    int cachedHarvestableMelons;

    BlockPos targetMelon;

    BlockPos pickupFocusMelonPos;
    ...
    public void reset() {
        ...
        cachedHarvestableMelons = 0;

        targetMelon = null;

        pickupFocusMelonPos = null;
    ...
    public void tick(Context ctx) {
        ...
        if (targetMelon != null && tryHandleHarvestTarget(ctx, targetMelon)) {
            return;
        }
    ...
    public int getCachedHarvestableMelons() {
        return cachedHarvestableMelons;
    }

    public boolean hasAnyCultureToDeposit() {
        return countItemInInventory(Items.MELON_SLICE) > 0;
    }

    public boolean shouldGoToStorageNow() {
        int slices = countItemInInventory(Items.MELON_SLICE);
        if (slices <= 0) {
            return false;
        }

        boolean inventoryTight = getFreeInventorySlots(36) <= 1;
        boolean enoughSlices = slices >= 64;
        boolean lotsOfSlices = slices >= 128;
        boolean noMoreTargetsNow = cachedHarvestableMelons <= 0;
        ...
        if (enoughSlices && noMoreTargetsNow) {
            return true;
        }
    ...
    public boolean dropNextDepositableCultureFromWholeInventory() {
        ...
        int melonSlot = findInventorySlotPreferWhole(Items.MELON_SLICE);
        if (melonSlot < 0) {
            return false;
        }

        return throwInventorySlot(melonSlot, true);
    }

    private void refreshTargets(Context ctx) {
        long now = System.currentTimeMillis();

        if (targetMelon != null && !isValidMelonTarget(ctx, targetMelon)) {
            if (targetMelon.equals(breakingMelonPos)) {
                resetBreakingState(true);
            }
            targetMelon = null;
        }
        ...
        if (targetMelon == null && now - lastFullRescanAt >= FULL_RESCAN_INTERVAL_MS) {
    ...
    private void scanLocalTargets(Context ctx) {
        ...
        double best = targetMelon != null ? playerPos.squaredDistanceTo(getInteractPoint(targetMelon)) : Double.MAX_VALUE;
        ...
        for (int y = fromY; y <= toY; y++) {
            for (int x = fromX; x <= toX; x++) {
                for (int z = fromZ; z <= toZ; z++) {
                    ...
                    if (!isHarvestableMelon(state)) {
                        continue;
                    }

                    if (!isValidMelonTarget(ctx, pos)) {
                        continue;
                    }
    ...
    private boolean tryHandleHarvestTarget(Context ctx, BlockPos melonPos) {
        if (!isValidMelonTarget(ctx, melonPos)) {
            if (melonPos.equals(breakingMelonPos)) {
                resetBreakingState(true);
            }
            targetMelon = null;
            return false;
        ...
        if (canReachBlock(melonPos)) {
            ctx.stopWorkPathing();
            resetTrackedGoal();

            boolean handled = tryHarvestMelon(melonPos);
            if (!handled) {
                return false;
            }
            ...
            if (!isValidMelonTarget(ctx, melonPos)) {
                targetMelon = null;
                pickupFocusMelonPos = melonPos.toImmutable();
        ...
        if (breakingMelonPos != null) {
            resetBreakingState(true);
        }

        BlockPos approach = findApproachPosForMelon(ctx, melonPos);
        if (approach == null) {
            pathToGoalTracked(ctx, findFallbackGoalNearMelon(ctx, melonPos));
            return true;
        }
    ...
    private boolean tryHandlePickupPhase(Context ctx) {
        if (pickupFocusMelonPos == null || mc.player == null || mc.world == null) {
            return false;
        }
        ...
        ItemEntity drop = findNearestRelevantDropAround(pickupFocusMelonPos, ITEM_SEARCH_RADIUS);
        if (drop == null) {
            return false;
        }
    ...
    private boolean isValidMelonTarget(Context ctx, BlockPos pos) {
        ...
        return isHarvestableMelon(mc.world.getBlockState(pos));
    }

    private boolean tryHarvestMelon(BlockPos melonPos) {
        if (mc.player == null || mc.interactionManager == null || mc.world == null || melonPos == null) {
            return false;
        }

        if (!isHarvestableMelon(mc.world.getBlockState(melonPos))) {
            if (melonPos.equals(breakingMelonPos)) {
                resetBreakingState(false);
            }
            return false;
        ...
        selectBestAxeForMelonIfAvailable();

        Direction dir = getBestBreakDirection(melonPos);
        long now = System.currentTimeMillis();

        try {
            if (breakingMelonPos == null || !breakingMelonPos.equals(melonPos)) {
                if (now < nextActionAt) {
                    return false;
                }
                ...
                lookAt(getInteractPoint(melonPos));

                mc.interactionManager.attackBlock(melonPos, dir);
                mc.player.swingHand(Hand.MAIN_HAND);

                breakingMelonPos = melonPos.toImmutable();
            ...
            boolean progressed;
            try {
                progressed = mc.interactionManager.updateBlockBreakingProgress(melonPos, dir);
    ...
    private void selectBestAxeForMelonIfAvailable() {
    ...
    private BlockPos findApproachPosForMelon(Context ctx, BlockPos melonPos) {
        return findApproachPosForInteract(ctx, melonPos, melonPos);
    }

    private BlockPos findFallbackGoalNearMelon(Context ctx, BlockPos melonPos) {
        if (ctx == null || melonPos == null || mc.world == null) {
            return melonPos;
        }
        ...
        return best != null ? best : melonPos;
    }
    ...
    private boolean isHarvestableMelon(BlockState state) {
        return state != null && state.isOf(Blocks.MELON);
    }
}