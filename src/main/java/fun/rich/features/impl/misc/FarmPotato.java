...
public class FarmPotato {
    ...
    static final int POTATO_RESERVE = 64;
    static final int EXTRA_POTATO_DELIVER_THRESHOLD = 128;
    ...
    int cachedMaturePotatoes;

    BlockPos targetPotatoCrop;
    BlockPos targetPotatoFarmland;
    ...
    public void reset() {
        ...
        cachedMaturePotatoes = 0;

        targetPotatoCrop = null;
        targetPotatoFarmland = null;
    ...
    public void tick(Context ctx) {
        ...
        if (!hasPotatoesInHotbar() && hasPotatoesAnywhere()) {
            pullPotatoesToHotbar();
        }

        if (targetPotatoCrop != null && tryHandleHarvestTarget(ctx, targetPotatoCrop)) {
            return;
        }

        if (targetPotatoFarmland != null && hasPotatoesAnywhere() && tryHandlePlantTarget(ctx, targetPotatoFarmland)) {
            return;
        }
    ...
    public int getCachedMaturePotatoes() {
        return cachedMaturePotatoes;
    }

    public boolean hasAnyCultureToDeposit() {
        return countItemInInventory(Items.POISONOUS_POTATO) > 0 || countPotatoesOverReserve() > 0;
    }

    public int countPotatoesOverReserve() {
        int reserve = getDynamicPotatoReserve();
        return Math.max(0, countItemInInventory(Items.POTATO) - reserve);
    }
    ...
    public boolean shouldGoToStorageNow() {
        ...
        int potatoes = countItemInInventory(Items.POTATO);
        int poisonous = countItemInInventory(Items.POISONOUS_POTATO);
        int reserve = getDynamicPotatoReserve();
        int potatoesOverReserve = Math.max(0, potatoes - reserve);

        boolean inventoryTight = getFreeInventorySlots(36) <= 1;
        boolean manyExtraPotatoes = potatoesOverReserve >= EXTRA_POTATO_DELIVER_THRESHOLD;

        if (poisonous > 0) {
            if (inventoryTight) {
                return true;
            }
            if (poisonous >= 16 && cachedMaturePotatoes <= 0) {
                return true;
            }
        }

        if (cachedEmptyFarmland > 0) {
            if (inventoryTight && (potatoesOverReserve > 0 || poisonous > 0)) {
                return true;
            }
            return false;
        }

        if (manyExtraPotatoes) {
            return true;
        }
        ...
        return potatoesOverReserve > 0 && cachedMaturePotatoes <= 0;
    }
    ...
    public boolean dropNextDepositableCultureFromWholeInventory() {
        ...
        int poisonSlot = findInventorySlotPreferWhole(Items.POISONOUS_POTATO);
        if (poisonSlot >= 0) {
            return throwInventorySlot(poisonSlot, true);
        }

        int reserve = getDynamicPotatoReserve();
        int totalPotatoes = countItemInInventory(Items.POTATO);
        int over = Math.max(0, totalPotatoes - reserve);
        if (over <= 0) {
            return false;
        }

        int wholeDropSlot = findInventorySlotPreferWholeAtMost(Items.POTATO, over);
        if (wholeDropSlot >= 0) {
            return throwInventorySlot(wholeDropSlot, true);
        }

        int potatoSlot = findInventorySlotPreferWhole(Items.POTATO);
        if (potatoSlot < 0) {
            return false;
        }

        return throwInventorySlot(potatoSlot, false);
    }

    private int getDynamicPotatoReserve() {
        int reserve = POTATO_RESERVE;
        ...
        if (cachedMaturePotatoes > 0 && cachedEmptyFarmland <= 0) {
    ...
    private void refreshTargets(Context ctx) {
        ...
        if (targetPotatoCrop != null && !isValidPotatoCropTarget(ctx, targetPotatoCrop)) {
            targetPotatoCrop = null;
        }
        if (targetPotatoFarmland != null && !isValidPotatoFarmlandTarget(ctx, targetPotatoFarmland)) {
            targetPotatoFarmland = null;
        }
        ...
        boolean needCrop = targetPotatoCrop == null;
        boolean needPlant = hasPotatoesAnywhere() && targetPotatoFarmland == null;
    ...
    private void scanLocalTargets(Context ctx) {
        ...
        double bestCrop = targetPotatoCrop != null ? playerPos.squaredDistanceTo(getInteractPoint(targetPotatoCrop)) : Double.MAX_VALUE;
        double bestFarmland = targetPotatoFarmland != null ? playerPos.squaredDistanceTo(Vec3d.ofCenter(targetPotatoFarmland.up())) : Double.MAX_VALUE;

        boolean canPlant = hasPotatoesAnywhere();
        ...
        for (int y = fromY; y <= toY; y++) {
            for (int x = fromX; x <= toX; x++) {
                for (int z = fromZ; z <= toZ; z++) {
                    ...
                    if (d < bestFarmland) {
                        bestFarmland = d;
                        targetPotatoFarmland = farmland.toImmutable();
                    }
                }
            }
        }
    ...
    private void scanFullTargets(Context ctx) {
        ...
        targetPotatoCrop = bestCropPos;

        if (hasPotatoesAnywhere()) {
            targetPotatoFarmland = bestFarmlandPos;
        } else {
            targetPotatoFarmland = null;
        }
    }
    ...
    private boolean tryHandleHarvestTarget(Context ctx, BlockPos cropPos) {
        if (!isValidPotatoCropTarget(ctx, cropPos)) {
            targetPotatoCrop = null;
            return false;
        }
    ...
    private boolean tryHandlePlantTarget(Context ctx, BlockPos farmlandPos) {
        if (!isValidPotatoFarmlandTarget(ctx, farmlandPos)) {
            targetPotatoFarmland = null;
            return false;
        }
    ...
    private ItemEntity findNearestRelevantDropAround(BlockPos centerPos, double radius) {
        ...
        for (Entity e : mc.world.getOtherEntities(null, box, ent -> ent instanceof ItemEntity)) {
            ...
            if (!stack.isOf(Items.POTATO) && !stack.isOf(Items.POISONOUS_POTATO)) {
                continue;
            }
    ...
    private boolean isValidPotatoCropTarget(Context ctx, BlockPos pos) {
        ...
        return isMaturePotato(mc.world.getBlockState(pos));
    }

    private boolean isValidPotatoFarmlandTarget(Context ctx, BlockPos pos) {
        if (pos == null || mc.world == null) {
            return false;
        }
    ...
    private boolean tryHarvestPotato(BlockPos cropPos) {
        if (mc.player == null || mc.interactionManager == null || mc.world == null) {
            return false;
        }
        if (!isMaturePotato(mc.world.getBlockState(cropPos))) {
            return false;
        }
    ...
    private boolean tryPlantPotato(BlockPos farmlandPos) {
        if (mc.player == null || mc.interactionManager == null || mc.world == null) {
            return false;
        }
        ...
        int potatoSlot = findHotbarSlot(Items.POTATO);
        if (potatoSlot < 0) {
            if (!pullPotatoesToHotbar()) {
                return false;
            }
            potatoSlot = findHotbarSlot(Items.POTATO);
            if (potatoSlot < 0) {
                return false;
            }
        }
        ...
        mc.player.getInventory().selectedSlot = potatoSlot;
    ...
    private boolean hasPotatoesInHotbar() {
        return findHotbarSlot(Items.POTATO) >= 0;
    }

    private boolean hasPotatoesAnywhere() {
        return countItemInInventory(Items.POTATO) > 0;
    }

    private boolean pullPotatoesToHotbar() {
        if (mc.player == null || mc.interactionManager == null) {
            return false;
        }

        if (findHotbarSlot(Items.POTATO) >= 0) {
            return true;
        }
        ...
        int potatoInvSlot = findInventorySlot(Items.POTATO, 9, 36);
        if (potatoInvSlot < 0) {
            return false;
        }

        int targetHotbar = findBestHotbarSlotForPotatoes();
        if (targetHotbar < 0) {
        ...
        try {
            ...
            int slotId = toScreenHandlerSlotId(potatoInvSlot);
    ...
    private int findBestHotbarSlotForPotatoes() {
        if (mc.player == null) {
            return -1;
        }
        ...
        for (int i = 0; i < 9; i++) {
            ItemStack s = mc.player.getInventory().getStack(i);
            if (s.isOf(Items.POTATO)) {
                return i;
            }
            if (s.isOf(Items.POISONOUS_POTATO)) {
                continue;
            }
            if (isFoodStack(s)) {
                continue;
    ...
    private boolean isMaturePotato(BlockState state) {
        if (state == null || !state.isOf(Blocks.POTATOES)) {
            return false;
        }
